...
export default async function MudurDevamsizlikPage() {
  ...
  const kayitlar = await prisma.attendance.findMany({
    where: { student: { institutionId }, tarih: { gte: otuzGunOnce } },
    include: { student: { select: { adSoyad: true, classroom: { select: { ad: true } } } } },
    orderBy: { tarih: "asc" },
  });
  ...
  const ogrenciMap = new Map<
    string,
    { adSoyad: string; sinifAdi: string | null; kayitlar: { durum: AttendanceStatus }[] }
  >();
  for (const kayit of kayitlar) {
    ...
    if (mevcut) mevcut.kayitlar.push(girdi);
      ...
      ogrenciMap.set(kayit.studentId, {
        ...
        sinifAdi: kayit.student.classroom?.ad ?? null,
        kayitlar: [girdi],
      });
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Kurum genelinde son 30 günlük devam durumu özeti.
        </p>
      </div>
              ...
              <TableBody>
                {ogrenciSirasi.map((o) => (
                  <TableRow key={o.studentId}>
                    ...
                    <TableCell>
                      <Link
                        href={`/mudur/ogrenci/${o.studentId}`}
